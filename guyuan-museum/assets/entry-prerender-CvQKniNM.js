const d="https://api.wuji.qq.com/x/api/wuji_cache/object?appid=my_buddy_test&schemaid=scene_pos_config_v2&schemakey=31bfc3054bb54a299ccf2c06a33162d9",n={"/":{title:"Web Wide World - 3D场景列表 | 高斯泼溅场景查看器",description:"Web Wide World 场景列表。提供多种高质量 3D Gaussian Splatting 重建场景，支持虚拟角色在场景中自由探索。By Nobody, For Everyone - 平凡人共创普惠世界。"},"/viewer2":{title:"场景查看器 - Web Wide World",description:"新一代3D场景查看器，支持高质量Gaussian Splatting渲染和物理交互。"},"/debug-viewer":{title:"Debug 场景查看器 - Web Wide World",description:"Debug调试工具面板，支持FPS监控、性能分析、侧边栏调试。"},"/creative":{title:"创意模式 - Web Wide World",description:"Creative Mode Test - 创意模式测试页面。"}};function l(e){return`
    ...
    </div>`}function s(e=3){return Array.from({length:e},()=>`
    ...
    </div>`).join("")}function c(e=[]){const i=e.filter(t=>t.modelUrlV2&&t.modelUrlV2!=="");return`
<div class="model-list">
  ...
  <div class="models-grid">
    ${i.length>0?i.map(t=>l(t)).join(""):s(3)}
  </div>
...
</div>`}async function p(){try{const e=await fetch(d);if(!e.ok)throw new Error(`请求失败: ${e.status} ${e.statusText}`);const i=await e.json(),o=Array.isArray(i?.data)?i.data:[];return console.log("[prerender] 场景列表加载成功，数量:",o.length),o}catch(e){return console.error("[prerender] 场景列表加载失败:",e),[]}}async function m(e){const i=e.url||"/",o=n[i]||n["/"];let t="";if(i==="/viewer2")t=a();else if(i==="/debug-viewer"||i==="/creative")t=a();else{const r=await p();console.log("[prerender] url:",i,"modelList length:",r.length),t=c(r)}return{html:t,head:{lang:"zh-CN",title:o.title,elements:new Set([{type:"meta",props:{name:"description",content:o.description}}])}}}export{m as prerender};
//# sourceMappingURL=entry-prerender-CvQKniNM.js.map
