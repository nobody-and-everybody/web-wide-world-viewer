const m="https://mogs-1300354879.cos.ap-shanghai.myqcloud.com/mogs/nobody/gs-viewer/config/scene.json",p="https://mogs-1300354879.cos.ap-shanghai.myqcloud.com/mogs/nobody/gs-viewer/config/scene_v2.json",d={"/":{title:"Web Wide World - 3D高斯泼溅场景查看器 | 平凡人共创普惠世界",description:"Web Wide World 是一个基于 3D Gaussian Splatting (3DGS) 技术的在线场景查看器。支持查看和探索高质量3D重建场景，提供沉浸式的虚拟世界体验。"},"/v2":{title:"Web Wide World V2 - 全新3D高斯泼溅场景体验",description:"Web Wide World V2 版本，提供更流畅的3D Gaussian Splatting场景浏览体验。探索虚拟世界，体验沉浸式3D场景。"},"/viewer2":{title:"场景查看器 - Web Wide World",description:"新一代3D场景查看器，支持高质量Gaussian Splatting渲染和物理交互。"},"/viewer":{title:"元宇宙查看器 - Web Wide World",description:"沉浸式元宇宙体验，控制虚拟角色在3D Gaussian Splatting场景中自由行走探索。"},"/spark":{title:"Spark 查看器 - Web Wide World",description:"基于 Spark 引擎的高性能3D Gaussian Splatting渲染器。"}};function g(i=[]){const t=i.filter(r=>r.modelUrlV2&&r.modelUrlV2!=="");return`
<div class="model-list">
  ...
  <div class="models-grid">
    ${t.length>0?t.map(r=>s(r)).join(""):c(3)}
  </div>
...
<div class="seo-content" style="position:absolute;left:-86.2rem;top:-86.2rem;">
  ...
  <nav>
    ...
    </div>`}function c(i=3){return Array.from({length:i},()=>`
...
<div class="model-list">
  ...
  <div class="models-grid">
    ${i.length>0?i.map(n=>s(n)).join(""):c(3)}
  </div>
...
</style>`}async function b(i){const t=i.url||"/",n=d[t]||d["/"];let r="",l={};if(t==="/v2"){let o=[];try{const e=await fetch(p);if(!e.ok)throw new Error(`请求失败: ${e.status} ${e.statusText}`);const a=await e.json();o=Array.isArray(a)?a:[]}catch(e){console.error("[prerender] V2场景列表加载失败:",e)}r=g(o),l={modelList:o}}else if(t==="/viewer2")r=h();else if(t==="/viewer"||t==="/spark"||t==="/simple-viewer")r=f(n.title);else{let o=[];try{const e=await fetch(m,{mode:"cors",credentials:"omit"});if(!e.ok)throw new Error(`请求失败: ${e.status} ${e.statusText}`);const a=await e.json();o=Array.isArray(a?.sceneConfig)?a.sceneConfig:[]}catch(e){console.error("[prerender] 首页场景列表加载失败:",e)}r=u(o),l={modelList:o}}return{html:r,data:l,links:new Set(["/","/v2","/viewer2","/viewer","/spark"]),head:{lang:"zh-CN",title:n.title,elements:new Set([{type:"meta",props:{name:"description",content:n.description}}])}}}export{b as prerender};
//# sourceMappingURL=entry-prerender-CMYH6Z0j.js.map
